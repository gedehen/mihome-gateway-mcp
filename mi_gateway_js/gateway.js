...
class Rr {
                ...
                this.#Q.addEventListener('message', async (e) => {
                    try {
                        if ('string' == typeof e.data)
                            return;  // Ignore text frames silently
                        this.#ce(await e.data.arrayBuffer())
                    } catch (_) {
                        // Ignore unparseable messages gracefully
                    }
                })
    }
    close() {
    ...
    async #ce(e) {
        ...
        switch (new DataView(e).getUint8(0)) {
            case Rr.DATA_TYPE.DATA:
                {
                    ...
                    const r = this.#ie.decrypt(e.slice(1)),
                        // Try primary decompress first (4-byte length prefix).
                        // If that yields gibberish, try without the length prefix.
                        i = (() => {
                            try {
                                return Br.decompress(r);
                            } catch (_) {
                                // Fallback: gateway may send raw deflate without length prefix
                                return _r.inflateRawSync(new Uint8Array(r)).buffer;
                            }
                        })()
                    let _rawStr = new TextDecoder().decode(i)
                    // The gateway may concatenate multiple JSON-RPC responses in one message
                    // (e.g., a push notification piggybacked on an API response).
                    // Extract the first complete JSON object rather than assuming
                    // the entire payload is a single JSON document.
                    // Also handles trailing non-printable padding/garbage bytes.
                    let _depth = 0, _start = -1, _inStr = false, _esc = false;
                    for (let _i = 0; _i < _rawStr.length; _i++) {
                        const _ch = _rawStr[_i];
                        if (_esc) { _esc = false; continue; }
                        if (_ch === '\\' && _inStr) { _esc = true; continue; }
                        if (_ch === '"') { _inStr = !_inStr; continue; }
                        if (_inStr) continue;
                        if (_ch === '{') { if (_depth === 0) _start = _i; _depth++; }
                        else if (_ch === '}') {
                            _depth--;
                            if (_depth === 0 && _start >= 0) {
                                _rawStr = _rawStr.substring(_start, _i + 1);
                                break;
                            }
                        }
                    }
                    const n = JSON.parse(_rawStr)
                    ...
                    if ('result' in n) {
                        ...
                        // Some gateway responses have both 'method' and 'id' fields.
                        // If the ID matches a pending request, treat it as a response
                        // (not a server-invoked method call).
                        const _pendingId = `${n.id}`
                        const _pendingReq = this.#oe.get(_pendingId)
                        if (void 0 !== _pendingReq) {
                            // This is actually a response to our request
                            // that happens to also have a 'method' field.
                            const responseData = n.params?.result ?? n.params ?? n.result
                            if (responseData !== undefined) {
                                try { clearTimeout(_pendingReq.timeoutHandle) } catch (t) { }
                                _pendingReq.resolve(responseData)
                            } else {
                                _pendingReq.reject(new Error(`Non-standard response: ${JSON.stringify(n).slice(0,200)}`))
                            }
                            this.#oe.delete(_pendingId)
                        } else {
            ...
            // Ignore unparseable messages (push notifications with binary data)
        }
    }
    #he(e) {
        if (!this.#ne) throw new Error('Secure session not established')
    ...
    #ye(e) {
        const t = e.slice(0, 16),
            r = e.slice(16, 24)
        this.#J = new h(t, r)
        // Expose session keys for HTTP API authentication
        this._sessionKey = t;
        this._sessionSalt = r;
